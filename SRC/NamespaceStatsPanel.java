...
import com.apelon.modules.dts.editor.namespaceplugin.StatFactory.Attribute;
import com.apelon.modules.dts.editor.namespaceplugin.StatFactory.StatObj;
...
class NamespaceStatsPanel extends JPanel  {
    ...
    private void runReport() {
    	...
    	try {
    		...
    		writeObjects("Namespace Properties", maps.get(Attribute.NAMESPACE_PROPS));
    		writeObjects("Versions with Properties", maps.get(Attribute.VERSION_PROPS));
    		writeObjects("Concepts with Properties", maps.get(Attribute.CONCEPT_PROPS));
    		writeObjects("Concepts with Synonyms", maps.get(Attribute.CONCEPT_SYNONYMS));
    		if (isOntylog) {
        		writeObjects("Concepts with Kinds", maps.get(Attribute.CONCEPT_KINDS));
        		writeObjects("Concepts with Roles", maps.get(Attribute.CONCEPT_ROLES));
        		writeObjects("Concepts with Inverse Roles", maps.get(Attribute.CONCEPT_INV_ROLES));
    		}
    		writeObjects("Concepts with Associations", maps.get(Attribute.CONCEPT_ASSNS));
    		writeObjects("Concepts with Inverse Associations", maps.get(Attribute.CONCEPT_INV_ASSNS));
    		writeObjects("Terms with Properties", maps.get(Attribute.TERM_PROPS));
    		writeObjects("Terms with Inverse Synonyms", maps.get(Attribute.TERM_INV_SYNONYMS));
    		writeObjects("Terms with Associations", maps.get(Attribute.TERM_ASSNS));
    		writeObjects("Terms with Inverse Associations", maps.get(Attribute.TERM_INV_ASSNS));
